## 2026/07/12 (純 EML モードで test3_v2 を実行)

### AI（エージェント・Claude Fable）が行ったこと
- ユーザー依頼で test3_v2 (28式 = A9新関数 + B9組合せ + C10スパース性、+1%ノイズ) を純 EML モード (pure_eml=True、Stage A/A2/C なし) で実行。`src/pure_eml_test3_v2.py` 新設 (共通モジュール流用、PURE_OVERRIDE で pure_eml/CPLX=10/各ステージ off)
- **結果: 4/28 ok (A 0/9・B 0/9・C 4/10)、変数特定 C 6/10、39分**。フル版 v5 の 25/28 を大きく下回るが、test3_v2 は v4 新演算子 (abs/sigmoid/min/max) 検証用で A/B 18式が純 EML の苦手構造 (非平滑・振動) のため想定どおり
- **構造クラスで綺麗に分離**:
  - 純単項式・単項式比 (C2, C10) と交互作用の和 (C4=x₁x₂+x₃x₄+x₅, C9=x₁x₈+x₂x₅) は ok・変数特定成功
  - abs/min (C6, C7)・sigmoid ゲート (C1) は failed/partial (実数 EML で表現不能)
  - 高次元 (C10: 25変数中5変数) でも乗法交互作用を回収・変数特定成功
- 応用含意をレポートに追記: 乗法・加法交互作用の発見は純 EML の得意領域で GWAS エピスタシス/PK 薬物相互作用の変数選択に直結。閾値応答・折れ線を含む場合はフル版か他ツールが必要
- ユーザーの技術質問への回答も実施: (a) 損失関数変更は数行では済まない (L2 が affine_fit/lstsq/LM に構造的に埋め込み、ErrorMetric::Custom は未接続)、(b) 損失変更だけでは Cox 非対応 (打ち切り・リスク集合・半パラメトリック性)、代替として martingale 残差法での構造発見を推奨。(c) PK では PySR が素直な既定解で pure_eml の優位は決定論・EML枠組み・Julia非依存に限る

### ユーザーが行ったこと
- test3_v2 を pure_eml で実行するよう依頼
- 純 EML の研究適性 (生存解析・薬物動態・遺伝解析)、損失関数変更の工数、Cox 対応可否、PK での PySR との比較を質問

---

## 2026/07/12 (純 EML モード解説書 manual_pure_eml-sr.md の作成)

### AI（エージェント・Claude Fable）が行ったこと
- ユーザーの理解確認に回答: 純 EML モードの本質は 2 機構 — (1) EML 基底ブースティング f=c0+Σcₖ·EML(Pₖ,Qₖ) (外側 cₖ は OLS=線形回帰、内側の基底関数 Pₖ,Qₖ を LM で 1 つずつ学習して足す射影追跡型)、(2) 純 EML 文法ビーム (加法形とは限らない任意木)。両候補を検証誤差で Pareto 選択し最良 1 式で予測。全予測が加法形ではないことを実際の発見式 (I.12.1=v0·v1、I.29.4=入れ子 EML) で示した
- `eml-sr_fable/manual_pure_eml-sr.md` を作成: 純 EML モードとは/理論 (EML 合成による表現力の導出・中心命題・振動構造の原理的限界)/アルゴリズム (2 機構と最終選択)/使い方 (pure_eml=True のクイックスタート、フル版との使い分け)/実績と限界 (31/99・79 式に EML 出現・アブレーション・I.40.1 の逆転例)/再現方法
- クイックスタート例を実行検証 (EML(…) を含む式が出力・RMSE 1e-3 を確認)

### ユーザーが行ったこと
- アルゴリズムの本質 (f=c0+Σcₖ·EML の線形回帰的な選択か、全予測がこの計算か) を質問
- 純 EML モードの理論・アルゴリズム・使い方をまとめた manual_pure_eml-sr.md の作成を依頼
