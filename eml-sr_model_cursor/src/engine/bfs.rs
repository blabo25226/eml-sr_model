...
pub fn run_bfs(
    ...
    // Seed tunable parameters with diverse initial values (0.5, 1.0, 3.0, ...).
    ...
    println!(
        "[EML-SR-Cursor] Started search space exploration. Level 1 seeded with {} primary nodes.",
        levels[1].len()
    );
    ...
    for k in 2..=config.max_complexity {
        ...
        println!(
            "[EML-SR-Cursor]   -> Level {} processed in {:?}. Retaining top {} candidates.",
            k,
            t_level.elapsed(),
            levels[k].len()
        );
...
fn run_refinement_pass(
    ...
    let candidates: Vec<(f64, Expression)> = {
    ...
    for (_err, expr) in sorted.into_iter().take(config.refinement_top_k) {
        if expr.param_count() == 0 {
            continue;
        }
        let (refined, error) = optimizer::refine_constants_multi(
            &expr,
            inputs,
            targets,
            registry,
        ...
        if error.is_finite() {
            update_pareto(front, &refined, error);
        }
    }
}
