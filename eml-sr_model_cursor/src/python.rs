...
create_exception!(eml_sr_model_cursor, EmlDimensionError, PyException);
create_exception!(eml_sr_model_cursor, EmlComplexityError, PyException);
create_exception!(eml_sr_model_cursor, EmlRuntimeError, PyException);
...
impl PySearcher {
    #[new]
    #[pyo3(signature = (max_complexity=7, complexity_penalty=0.08, beam_width=1200))]
    fn new(max_complexity: usize, complexity_penalty: f64, beam_width: usize) -> Self {
        let mut config = SearchConfig::cursor_default();
...
#[pymodule(name = "eml_sr_model_cursor")]
fn eml_sr_model_cursor(py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
