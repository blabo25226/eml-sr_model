...
//! `eml-sr-fable` — Enhanced symbolic regression via the EML operator.
...
impl Searcher {
    ...
    pub fn find_function(&self, xs: &[f64], ys: &[f64]) -> Result<SearchResult, EmlError> {
        ...
        let mut results = engine::fable::run_fable(&inputs, ys, &self.config)?;
        Ok(results.remove(0))
    }
    ...
    pub fn find_candidates(
        ...
        engine::fable::run_fable(inputs, ys, &config)
    }
}
