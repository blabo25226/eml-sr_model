...
#[cfg(feature = "full-math")]
unary_op!(Square, "Square", value::c_square, r"{{{0}}}^2",      "(({0})**2)",    "Power[{0}, 2]");
#[cfg(feature = "full-math")]
unary_op!(Cube,   "Cube",   value::c_cube,  r"{{{0}}}^3",      "(({0})**3)",    "Power[{0}, 3]");
#[cfg(feature = "full-math")]
...
impl Operator for Pow {
    ...
    fn arity(&self) -> u8 { 2 }
    fn eval(&self, args: &[Value]) -> Option<Value> { value::c_pow(args[0], args[1]) }
    fn to_latex(&self, args: &[&str]) -> String { format!(r"{{{0}}}^{{{1}}}", args[0], args[1]) }
    fn to_python(&self, args: &[&str]) -> String { format!("(({0})**({1}))", args[0], args[1]) }
}
